...
class ElasticsearchServiceTestContext:
    def __init__(self):
        config = TestConfigLoader.load_config()
        self.es_index_0 = config[ELASTICSEARCH_INDEX_0]
        self.es_index_1 = config[ELASTICSEARCH_INDEX_1]
...
class TestElasticsearchServiceCommon(TestCase, ElasticsearchServiceTestContext):
    ...
    def test_get_indices(self):
        ...
        for index in [self.es_index_0, self.es_index_1]:
            resp = self.es_impl.get_indices(self.es_index_1)
...
class TestElasticsearchIndex0(TestCase, ElasticsearchServiceTestContext):
    ...
    def test_delete_index(self):
        resp = self.es_impl.delete_index(index=self.es_index_0)
        print(f'delete index: {resp}')

    def test_setup(self):
        resp = self.es_impl.setup_index_0(self.es_index_0)
        pprint(resp)

    def test_count(self):
        pprint(self.es_impl.count(self.es_index_0))


class TestElasticsearchIndex1(TestCase, ElasticsearchServiceTestContext):
    def __init__(self, *args, **kwargs):
        ElasticsearchServiceTestContext.__init__(self)
        TestCase.__init__(self, *args, **kwargs)

    def test_delete_index(self):
        resp = self.es_impl.delete_index(index=self.es_index_1)
        print(f'delete index: {resp}')

    def test_setup(self):
        pprint(self.es_impl.setup_index_1(self.es_index_1))

    def test_count(self):
        pprint(self.es_impl.count(self.es_index_1))
...
class TestElasticsearchServiceImpl(TestCase, ElasticsearchServiceTestContext):
    def __init__(self, *args, **kwargs):
        ElasticsearchServiceTestContext.__init__(self)
        TestCase.__init__(self, *args, **kwargs)

    def test_reindex(self):
        src_index = self.es_index_0
        dst_index = self.es_index_1
